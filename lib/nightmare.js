...
Nightmare.prototype.run = function(fn) {
  ...
  function next (err, res) {
    self.nightmarePreviousActionResult = res;
    ...
    var cleanStackArray = error.stack.split('\n');
    if (cleanStackArray.length > 3) {
        cleanStackArray = cleanStackArray.slice(1, -2);
    } else if (cleanStackArray.length > 1) {
        cleanStackArray = cleanStackArray.slice(1);
    }
    var methodName = error.message ? error.message : "next method";
    console.error('running ' + methodName + '\n' + cleanStackArray.join("\n"));
  ...
  function after (err, res) {
    ...
    if(err) {
        if(self.child) {
            actions.screenshot.call(self, './dotco/www/tests/electron/screenshots/latest.png', callNext);
...
Nightmare.prototype.queue = function(done) {
  ...
  var args = sliced(arguments);
  var fn = args.pop();
  var error = this.debugLog ? this.debugLog : new Error();
  this.debugLog = undefined;
  this._queue.push([fn, args, error]);
};
...
function queued (name, fn) {
  return function action () {
    ...
    var error = this.debugLog ? this.debugLog : new Error(name);
    this.debugLog = undefined;
    this._queue.push([fn, args, error]);
    return this;
  }
